...
- Production needs migration `005` applied (`npm run db:migrate`) before the
  add/remove API works; the seed-only view degrades gracefully until then. The
  migration runner now tracks applied migrations in `schema_migrations` and is
  idempotent, so `db:migrate` is safe to re-run (it bootstraps the 001–004
  baseline on DBs that predate tracking and then applies only 005).
...
header button (plus an idempotent `db:migrate` runner with a `schema_migrations`
tracking table). No feature flag — additive and auth-gated. Run `npm run
db:migrate` against the production Neon DB and ensure `GROQ_API_KEY` is set on
Vercel as part of deploy. The seed list renders immediately; add + Groq
generation activate once the migration and key are live.
