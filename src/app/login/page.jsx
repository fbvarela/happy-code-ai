...
export default async function LoginPage({ searchParams }) {
  ...
  const error = params?.error ? ERRORS[params.error] || "Error de autenticación." : null;
  ...
  return (
    <main style={{ minHeight: "100vh", display: "grid", placeItems: "center", padding: 20 }}>
      <div className="card" style={{ padding: 32, width: "100%", maxWidth: 400, textAlign: "center" }}>
        ...
        <p style={{ color: "var(--text-muted)", marginBottom: 24 }}>Agent Artifact Manager</p>

        {error && (
          <p style={{ color: "var(--clay)", marginBottom: 16, fontSize: "0.9rem" }}>{error}</p>
        )}
        ...
        <p style={{ marginTop: 16, fontSize: "0.8rem", color: "var(--text-muted)" }}>
          Pedimos el permiso <code>repo</code> para poder publicar artefactos en tus repositorios.
        </p>
      </div>
    </main>
  );
