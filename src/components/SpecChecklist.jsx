...
import { lintSpec, SPEC_QUALITY_KEYS, SPEC_AUTO_KEYS } from "@/lib/quality";

export default function SpecChecklist({ body }) {
  const { t } = useI18n();
  const [open, setOpen] = useState(false);
  const [manual, setManual] = useState({});
  const results = useMemo(() => lintSpec(body), [body]);

  const passed = SPEC_QUALITY_KEYS.filter((k) => {
    ...
    <div style={{ marginTop: 16, borderTop: "1px solid var(--line)", paddingTop: 12 }}>
      ...
      >
        <span style={{ fontSize: "0.9rem", fontWeight: 600 }}>
          {t("spec.checklist.title")}
          <span style={{
            marginLeft: 8, fontSize: "0.75rem", fontWeight: 400,
            color: passed === SPEC_QUALITY_KEYS.length ? "var(--leaf)" : "var(--text-muted)",
          }}>
            {passed}/{SPEC_QUALITY_KEYS.length}
          </span>
        </span>
      ...
      {open && (
        <div style={{ marginTop: 10 }}>
          <p style={{ fontSize: "0.75rem", color: "var(--text-muted)", marginBottom: 8 }}>
            {t("spec.checklist.hint")}
          </p>
          <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "grid", gap: 5 }}>
            {SPEC_QUALITY_KEYS.map((k) => {
              const auto = results[k];
              const isAuto = auto !== null && k !== "_meta";
              ...
              return (
                <li key={k} style={{ display: "flex", alignItems: "flex-start", gap: 7 }}>
                  ...
                  <span style={{ fontSize: "0.82rem", lineHeight: 1.45, color: checked ? "var(--text)" : "var(--text-muted)" }}>
                    ...
                    {k === "specificity" && (
                      <span style={{ marginLeft: 5, fontSize: "0.7rem", color: "var(--text-muted)", opacity: 0.7 }}>
                        ({meta.vagueCount || 0} {t("spec.check.vagueFound")})
                      </span>
                    )}
                    {isAuto && (
                      <span style={{ marginLeft: 5, fontSize: "0.7rem", color: "var(--text-muted)", opacity: 0.7 }}>
